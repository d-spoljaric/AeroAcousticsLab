...
mach_range = np.arange(0.05, 0.65, 0.05)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(observer_theta_list)), desc = "Looping over theta"):
        for count, mach in tqdm(enumerate(mach_range), desc = "Looping over Mach"):
            thrust_total = 2000 # [N]
            r_thrust = 0.85 # [-], location of force as a fraction of propeller radius
            M_tip = mach # [-], propeller tip Mach number
            ...
            period = (1/s)*2*np.pi/vel_rad # Do not edit this
            ...
            p_fluctuations = np.zeros(shape = (len(t_observer_array), len(mach_range)))
        ...
        if plot_per_theta:
            for i in range(len(mach_range)):
                plt.plot(mach_range, p_fluctuations[:, i], label = f"Mach {mach_range[i]}")
            plt.legend()
            plt.minorticks_on()
