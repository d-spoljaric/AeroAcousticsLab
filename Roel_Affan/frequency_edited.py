...
def pressure_const_force(input_par, phi, theta):
    a = -1j
...
def freq_to_spl(p_freq):
    p_rms = np.sqrt(np.mean(np.real(sum(p_freq**2))))  # calculate prms
    p_ref = 2e-5    # Defined reference pressure
...
theta_list = np.deg2rad(np.arange(0, 90, 1))
p_mB_array = np.zeros(shape = (len(theta_list), len(m)))
for i in tqdm(range(len(theta_list))):
    for j in tqdm(range(len(m))):
        theta = theta_list[i]
        m_temp = m[j]
        p = frequency(inp, phi, theta, m_temp, inp.s)
        p_mB_array[i, j] = p

plt.scatter(m, p_mB_array[45, :])
plt.xlabel("m")
plt.ylabel("p")
plt.show()

SPL_theta = np.zeros(shape = theta_list.shape)
for i in range(len(SPL_theta)):
    SPL_theta[i] = freq_to_spl(p_mB_array[i, :])
    
plt.scatter(theta_list, SPL_theta)
plt.show()
